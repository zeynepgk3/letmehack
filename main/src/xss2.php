...
if (isset($_POST['xss2'])) {
    if ($_SESSION[$_SESSION['uid']]['up'] == 1) {
        exec("sudo /usr/local/bin/docker rm -rf " . $_SESSION['uid'] . "-xss2", $output, $retval);
        $_SESSION[$_SESSION['uid']]['up'] = 0;
    } else {
        exec("sudo /usr/local/bin/docker run --rm -d --name " . $_SESSION['uid'] . "-xss2 -p 0:80 xss2", $output, $retval);
        $_SESSION[$_SESSION['uid']]['up'] = 1;
    }
}
if (isset($_SESSION[$_SESSION['uid']]['up'])) {
    exec("sudo /usr/local/bin/docker inspect --format='{{range \$p, \$conf := .NetworkSettings.Ports}}{{(index \$conf 0).HostPort}} {{end}}' " . $_SESSION['uid'] . "-xss2", $port, $retval);
}
?>
        ...
        <div class="row">
            ...
            <div class="col-lg-8 mb-5 mb-lg-0">
                ...
                <div class="comments-form border-box">
                    ...
                    <form action="?" name="xss2" method="POST">
                        <div class="clearfix">
                        ...
                        <?php
                            if (@$_COOKIE['userInfo']) {
                                if (!isset($_SESSION[$_SESSION['uid']]['up'])) {
                            ?>
                                    <button class="btn btn-success" type="submit" aria-label="post-comment" name="xss2">Labı Başlat</button>
                                <?php
                                } else {
                                ?>
                                    <button class="btn btn-danger" type="submit" aria-label="post-comment" name="xss2">Labı Kapat</button>
                                <?php
                                }
                            } else {
                                ?>
                    ...
                    ?>
                            ...
                            <a target="_blank" href="http://localhost:<?php echo $port[0]; ?>/xss2.php">http://localhost:<?php echo $port[0]; ?> </a>
                    <?php
                        }
                    }
                    ?>
            ...
            <div class="col-lg-4 mb-5 mb-lg-0">
                <div class="post-media post-image">
                    <img loading="lazy" src="assets/images/xss_picture.png" class="img-fluid" alt="post-image">
                </div>
            </div>
