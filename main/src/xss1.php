...
if (isset($_POST['xss1'])) {
    if ($_SESSION[$_SESSION['uid']]['up'] == 1) {
        exec("sudo /usr/local/bin/docker rm -rf " . $_SESSION['uid'] . "-xss1", $output, $retval);
        $_SESSION[$_SESSION['uid']]['up'] = 0;
    } else {
        exec("sudo /usr/local/bin/docker run --rm -d --name " . $_SESSION['uid'] . "-xss1 -p 0:80 xss1", $output, $retval);
        $_SESSION[$_SESSION['uid']]['up'] = 1;
    }
}
if (isset($_SESSION[$_SESSION['uid']]['up'])) {
    exec("sudo /usr/local/bin/docker inspect --format='{{range \$p, \$conf := .NetworkSettings.Ports}}{{(index \$conf 0).HostPort}} {{end}}' " . $_SESSION['uid'] . "-xss1", $port, $retval);
}
?>
        ...
        <div class="row">
            ...
            <div class="col-lg-8 mb-5 mb-lg-0">
                ...
                <div class="post-content post-single">
                    ...
                    <div class="post-body">
                        ...
                        <div class="entry-content">
                            ...
                            Lab Açıklaması <br>
                            Senden vereceğimiz lab içerisinde XSS zafiyetinin olup olmadığını bulmanı istiyorum. Gazan mübarek olsun :) <br>
                            </p>
                        </div>
                ...
                <div class="comments-form border-box">
                    ...
                    <form action="?" name="xss1" method="POST">
                        <div class="clearfix">
                        ...
                        <?php
                            if (@$_COOKIE['userInfo']) {
                                if (!isset($_SESSION[$_SESSION['uid']]['up'])) {
                            ?>
                                    <button class="btn btn-success" type="submit" aria-label="post-comment" name="xss1">Labı Başlat</button>
                                <?php
                                } else {
                                ?>
                                    <button class="btn btn-danger" type="submit" aria-label="post-comment" name="xss1">Labı Kapat</button>
                                <?php
                                }
                            } else {
                                ?>
                    ...
                    ?>
                            ...
                            <a target="_blank" href="http://localhost:<?php echo $port[0]; ?>/xss1.php">http://localhost:<?php echo $port[0]; ?> </a>
                    <?php
                        }
                    }
                    ?>
